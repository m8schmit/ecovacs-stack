...
import { getVacuumMap, getVacuumPos } from '../../store/vacuum/vacuumSlice';
...
const VacuumMap = () => {
  ...
  const getCoordinates = (value: number, axis: 'x' | 'y') => (axis === 'x' ? mapWidth : mapHeigth) / 2 + value;
  ...
  const [dockLayer] = useState(
    new VectorLayer({
      ...
      source: new Vector({
        features: [
          new Feature({
            geometry: new Point([getCoordinates(0, 'x'), getCoordinates(0, 'y')]),
            finished: false,
            name: 'Charging dock',
          }),
        ],
  ...
  useEffect(() => {
    if (map && mapData && mainLayer) {
      ...
      console.log(mainLayer.getSource()?.get);
